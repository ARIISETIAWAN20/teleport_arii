-- ✅ Arii Teleport GUI FINAL | HWID Lock + Cheat Defend + Auto Kick Chat | Delta Safe
...
-- 🔎 Light Deep Scan (workspace only, Highlight only)
RunService.RenderStepped:Connect(function()
    for _, obj in pairs(workspace:GetDescendants()) do
        if obj:IsA("Highlight") then
            if obj.Adornee and obj.Adornee:IsA("Model") and not obj.Adornee:IsDescendantOf(player.Character) then
                player:Kick("Observer/Admin tersembunyi terdeteksi (workspace)")
            end
        end
    end
end)
