...
local delayTime = 8
local lastSaved1, lastSaved2 = nil, nil
	...
	while true do
		...
		if hrp then
			...
			if teleportPoints.point1 then
				local p1 = Vector3.new(teleportPoints.point1.x, teleportPoints.point1.y, teleportPoints.point1.z)
				if (pos - p1).Magnitude > 10 then
					teleportPoints.point1 = {x=pos.X, y=pos.Y, z=pos.Z}
					savePoints()
				end
			end
			if teleportPoints.point2 then
				local p2 = Vector3.new(teleportPoints.point2.x, teleportPoints.point2.y, teleportPoints.point2.z)
				if (pos - p2).Magnitude > 10 then
